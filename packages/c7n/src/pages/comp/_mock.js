...
const dsQueriesSize10Page1T = {
  rows: [...dataBlock1, ...dataBlock2],
  total: 20,
  success: true,
};
const dsQueriesSize10Page2T = {
  rows: [...dataBlock3, ...dataBlock4],
  total: 20,
  success: true,
};

const dsLargeQueriesT = {
  rows: new Array(50).fill(null).reduce(
    (rows, _empty, index) =>
      rows.concat(
        dsQueriesT.rows.map((item, useid, size) => ({
...
const treeMock = Mock.mock(TreeData);
