...
import { getThemeData, hexToRgbaColor } from '@hzero-front-ui/core/lib/utils/utils';
import { createStyleComponent, createMainStyleComponent, ThemeContext } from '@hzero-front-ui/core';
import { hzeroFontSize, vars } from './utils';
...
export const c7nPolyfillStyleCss = css`
...
const C7nPolyfillStyleComponent = createMainStyleComponent({
  name: 'c7n-ued-polyfill',
  styles: [createStyleComponent('c7n-ued-polyfill', c7nPolyfillStyleCss, true)],
});


const C7nPolyfill = () => {
  const { schema, resolvedTheme, config } = useContext(ThemeContext);
  const shouldUsePolyfill = !config || schema === 'theme2';
  return (
    <>
      {shouldUsePolyfill ? <C7nPolyfillStyleComponent data={resolvedTheme} /> : null}
    </>
  );
};

export default C7nPolyfill;
